...
- **Line 567**: `_ToolCallCard` (Now a **StatefulWidget**)
  - **Expandable Interface**: Clicking a tool card now toggles its visibility.
  - **Compact State**: Shows the tool name and its `arguments` in a single-line "JetBrains Mono" font (e.g., `google_gmail_read {"max_results": 1}`).
  - **Expanded State**: Reveals a `SelectableText` area containing the full JSON output of the tool. This follows the pattern of modern coding agents like Claude, where detailed technical logs are hidden behind a click to keep the chat clean.
